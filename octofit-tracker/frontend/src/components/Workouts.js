...
export default function Workouts() {
  ...
  // Codespace API URL: https://codespace-8000.app.github.dev/api/workouts/
  ...
  async function fetchData() {
    ...
    try {
      ...
      setWorkouts(normalizeResult(json));
    } catch (err) {
  ...
  return (
    <div className="card shadow-sm mb-4">
      ...
      <div className="card-body">
        ...
        <p className="mb-3">Loaded: <strong>{workouts.length}</strong> records</p>

        {error && <div className="alert alert-danger">Error: {error}</div>}

        {loading ? <div className="alert alert-info">Loading data...</div> : <DataTable items={workouts} onSelect={setSelected} />}
