...
export default function Leaderboard() {
  ...
  async function fetchData() {
    ...
    try {
      ...
      setLeaderboard(normalizeResult(json));
    } catch (err) {
  ...
  return (
    <div className="card shadow-sm mb-4">
      ...
      <div className="card-body">
        ...
        <p className="mb-3">Loaded: <strong>{leaderboard.length}</strong> records</p>

        {error && <div className="alert alert-danger">Error: {error}</div>}

        {loading ? <div className="alert alert-info">Loading data...</div> : <DataTable items={leaderboard} onSelect={setSelected} />}
